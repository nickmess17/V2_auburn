function [ sediment_bioirrigation_fluxes, sediment_transport_fluxes, sediment_concentrations, sediment_additional_results] = sediment_v2(sediment_concentrations, sediment_params, sediment_matrix_templates, sediment_bc)
  ...
  for i=2:m
    ...
    a = alphaxxx(-log10(H3O)+3, sediment_params.aq_system.carb_acid.pKs);
    CO2(:,i-1) = Ct.*a(:,1);
    HCO3(:,i-1) = Ct.*a(:,2);
    CO3(:,i-1) = Ct.*a(:,3);

    Nt = NH3(:,i-1) + NH4(:,i-1);
    a = alphaxxx(-log10(H3O)+3, sediment_params.aq_system.amonia.pKs);
    NH4(:,i-1) = Nt.*a(:,1);
    NH3(:,i-1) = Nt.*a(:,2);

    St = HS(:,i-1) + H2S(:,i-1);
    a = alphaxxx(-log10(H3O)+3, sediment_params.aq_system.sulf.pKs);
    H2S(:,i-1) = St.*a(:,1);
    HS(:,i-1) = St.*a(:,2);

    % -log10(H3O)+3

    a = alphaxxx(-log10(H3O)+3, sediment_params.aq_system.p_acid.pKs);
