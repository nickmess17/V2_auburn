...
lake_params = {
    % PhysPar
    1, 'dz',                 % 1
    0.0185, 'Kz_K1',     % 2     open water diffusion parameter (-)
    ...
    44.15, 'lat',              % 6     latitude (decimal degrees)
    -70.25, 'lon',              % 7     longitude (decimal degrees)
    0.3, 'alb_melt_ice',       % 8     albedo of melting ice (-)
    0.77, 'alb_melt_snow',     % 9     albedo of melting snow (-)
    0.00005, 'PAR_sat',       % 10    PAR saturation level for phytoplankton growth (mol(quanta) m-2 s-1)
    ...
    0.2, 'F_sed_sld',         % 15    volume fraction of solids in sediment (= 1-porosity)
    1.8911, 'I_scV',                % 16    scaling factor for inflow volume (-)
    0.1276, 'I_scT',                % 17    adjusting delta for inflow temperature (-)
    ...
    0.3398, 'swa_b0',             % 39     non-PAR light attenuation coeff. (m-1)
    1.5623, 'swa_b1',            % 40     PAR light attenuation coeff. (m-1)
    3.63e-07, 'S_res_epi',     % 41     Particle resuspension mass transfer coefficient, epilimnion (m day-1, dry)
    3.63e-08, 'S_res_hypo',    % 42     Particle resuspension mass transfer coefficient, hypolimnion (m day-1, dry)
    0.02, 'H_sed',             % 43     height of active sediment layer (m, wet mass)
    1000, 'Psat_L',              % 44     NOTE: NOT USED: Half saturation parameter for Langmuir isotherm
    5000, 'Fmax_L',              % 45     NOTE: NOT USED: Scaling parameter for Langmuir isotherm !!!!!!!!!!!!
    0.03, 'w_s',                % 46     settling velocity for S (m day-1)
    0.1, 'w_chl',             % 47     settling velocity for Chl a (m day-1)
    0.5, 'Y_cp',                 % 48     NOTE: NOT USED:  yield coefficient (chlorophyll to carbon) * (carbon to phosphorus) ratio (-)   1/55*112/1 = 1
    0.1, 'm_twty',             % 49    loss rate (1/day) at 20 deg C
    ...
    0.5, 'P_half',             % 53    Half saturation growth P level (mg/m3)
...
sediment_params = {
    ...
    0.04,  'k_DOP',                 % 4       % 1
    0.008, 'k_DOC',                  % 5       % 1
    0.002, 'Km_O2',                 % 6       % Canavan, R. W (2006) rho=2.5
    ...
    33,     'effective_depth',     % 72           % depth below which the lake is affected by sediments, [m], if -1 (experimental) , then sediments below pycnocline
    ...
    0,    'SO4 flux',             % 75           % default 0; flux of sulphate from bottom of the sediment. Custom boundary condition for Lake Vansjo only
    };
